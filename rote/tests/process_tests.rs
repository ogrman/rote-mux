...
async fn test_panel_stderr_buffer() {
    ...
    let mut proc = TaskInstance::spawn(PanelIndex::new(0), &cmd, None, tx, shutdown_tx.subscribe())
        .expect("spawn failed");
...
async fn test_terminate_multiple_processes() {
    ...
    let mut proc1 = TaskInstance::spawn(
        PanelIndex::new(0),
        &cmd1,
        None,
        tx.clone(),
    ...
    .expect("spawn failed");
    let mut proc2 = TaskInstance::spawn(
        PanelIndex::new(1),
        &cmd2,
        None,
        tx.clone(),
    ...
    .expect("spawn failed");
    let mut proc3 = TaskInstance::spawn(
        PanelIndex::new(2),
        &cmd3,
        None,
        tx.clone(),
